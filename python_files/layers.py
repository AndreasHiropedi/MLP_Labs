...
class AffineLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        dot_products = inputs.dot(weights_transposed)
        outputs = dot_products + self.biases
        return outputs
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        # Using dot product instead of matmul
        # we also need to transpose the inputs to make the multiplication possible
        # and we then need to transpose back to have the correct dimensions
        grads_wrt_weights = inputs.T.dot(grads_wrt_outputs).T 

        # Calculate gradients with respect to bias
        grads_wrt_bias = np.sum(grads_wrt_outputs, axis=0)

        return grads_wrt_weights, grads_wrt_bias
